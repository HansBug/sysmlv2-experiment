...
| [OMG SysML v2 Pilot Implementation](https://github.com/Systems-Modeling/SysML-v2-Pilot-Implementation) | 官方参考实现与验证示例 | `sysml/src/**/*.sysml`、`sysml.library`；与官方 Pilot JAR 配套 | 固定 commit 含 255 个 `.sysml`，其中 16 个文件含 47 个通过源校验的状态根；当前独立批次有 6 个 converted |
...
| [elan8/sysml-examples](https://github.com/elan8/sysml-examples) | Traffic Light、Kitchen Timer、Surveillance Drone 等公开行为示例 | 固定提交 `fdedcc0d25f45ce0fc54b5650b5defaf3c64d719`，仓库内 `*.sysml`；MIT | 已下载 24 个文件；完整工程上下文中 4 个控制状态候选全部通过当前 FCSTM 目标检查，适合作为可复现的正向行为样本 |
| [MBSE4U/dont-panic-batmobile](https://github.com/MBSE4U/dont-panic-batmobile) | SysML v2 入门书籍中的 Batmobile 示例 | 固定提交 `f6cdf3577397eb290805b733811a9548c7ce2cfb`，`DontPanic-SysMLv2-Batmobile.sysml`；Apache-2.0 | 已取得 1 个模型文件；当前 Pilot 报告 `Subject must be first parameter` 约束错误，作为旧语法/约束兼容性样本保留，未计入转换分母 |
| [CMBSE（TU Ilmenau）](https://github.com/ziruili-tu-ilmenau/CMBSE)；论文 [10.3390/systems12010018](https://doi.org/10.3390/systems12010018) | 论文中的协同 MBSE、力测量系统和供应商模型 | 固定提交 `42a8a0d2487d64cfbe20e1cf23a0421dde748ad0`，12 个 `*.sysml`；仓库未声明 SPDX 许可证 | 论文关联模型资产已取得；当前 Pilot 独立加载均出现旧式 `import`/链接诊断，需匹配作者工具链复核，暂不进入严格转换分母 |
| [elan8/sysml-robot-vacuum-cleaner](https://github.com/elan8/sysml-robot-vacuum-cleaner) | 自主清洁机器人系统工程展示模型 | 固定提交 `a999fa8f4bafb8cf159925796b09b65f6bac7f2b`，13 个 `*.sysml`；MIT | 已取得 13 个模型文件；当前模型依赖额外 `sysml-domain-libraries`/`mbse-methodology`，官方 Pilot 单独加载未通过，作为工程上下文依赖样本登记 |
