...
这些集合多用于语言建模、生成或 conformance；不都是可执行控制器，更不是等量独立系统。文件级 SHA256 用于复查重复，当前 1,669 文件包含 1,502 种内容；新增 Advent、Apollo 11 和 SysML v2 Book 示例均按固定提交记录。Book 示例在当前 Pilot 下有完整源文件校验错误，因此未进入 typed 状态根转换分母。
