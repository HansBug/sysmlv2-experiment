...
当前对未参与控制拓扑的 `ReferenceUsage`、`PartUsage`、`PortUsage` 采用显式 `ignored_structural` 登记；用于 action 参数、消息 receiver 或端口通道的引用保留为 opaque action channel 元数据。已增加一个严格的 typed FeatureChain 子集：单实例 part/reference 到标量数值 feature 的一层链可映射为带结构来源的 FCSTM 变量，外部链使用零值输入近似，并验证初始化、赋值和 guard 使用同一链；多实例、数组、跨对象和更深链仍拒绝，避免把结构成员的行为依赖误当成可忽略元数据。
