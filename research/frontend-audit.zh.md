...
接入官方 Pilot 和 Apollo 11 后，CI 扫描了 77 个上下文、265 个上下文内文件，得到 92 个状态根，其中 52 个控制状态候选，10 个 converted、82 个明确 unsupported。Apollo 11 作为完整 28 文件根目录加载时为 0 条校验错误、18 个状态根；此前按子目录拆分造成的 842 条错误已确认是我们的上下文分组缺陷。该结果包含同一概念在不同公开数据集中的重复版本，不能按状态根数宣称独立系统数量；汇总见 [`project-context-ci-observed.json`](project-context-ci-observed.json)。
